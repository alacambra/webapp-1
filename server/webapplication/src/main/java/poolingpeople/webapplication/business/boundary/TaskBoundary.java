...
public class TaskBoundary {
	...
	//TODO inject with registerModule(setMixInAnnotation) annotation
	...
	public class TaskMixinModule extends SimpleModule {
		...
		public void setupModule(SetupContext context) {
			context.setMixInAnnotations(Task.class, TaskMixin.class);
		}
	}
	...
	public Response getTask(@PathParam("id") String id) {
		try {
			Task task = entityFactory.getTask(id);
			String r = mapper.writerWithView(TaskMixin.class)
			...
			throw new WebApplicationException(e, Response
					...
					.entity("Task with ID " + id + " not found").build());
		} catch (Exception e) {
	...
	public Response getAllTask() {
		try {
			...
			String r;
			r = mapper.writerWithView(View.SampleView.class)
					.writeValueAsString(entityFactory.getAllTask());
			return Response.ok().entity(r).build();
		} catch (JsonGenerationException e) {
			throw new WebApplicationException(e, Response
					.status(Status.BAD_REQUEST)
					.entity("invalid Json. Deserialize not possible").build());
	...
	public Response updateTask(@PathParam("id") String uuid, String json) {
		...
		try {
			...
			throw new WebApplicationException(e, Response
					.status(Status.NOT_FOUND).entity(e.getMessage()).build());
		} catch (Exception e) {
			throw new WebApplicationException(e.getMessage(), e);
		}
	}
